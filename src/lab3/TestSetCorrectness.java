...
public class TestSetCorrectness {
    ...
    public static void testSet(int n1, int n2, int n3, int n4) throws Exception {
        ...
        if(n1 == 1){
            ...
            throw new Exception("Not a valid n1.");
        }
        for(int i = 1; i <= n2; i++) {
            System.out.println("Restart nbr: " + i);
            for (int k = 1; k <= n3; k++) {
                ...
                if (randomInt == 0) {
                    ...
                    if (set.size() != refSet.size()) {
                        throw new Exception("Bug in size method.");
                    }
                } else if (randomInt == 1) {
                    ...
                    if (set.add(n4Rand) != refSet.add(n4Rand)) {
                        throw new Exception("Bug in add method.");
                    }
                } else if (randomInt == 2) {
                    int n4Rand = rand.nextInt(n4 - 1);
                    ...
                    if (set.remove(n4Rand) != refSet.remove(n4Rand)) {
                        throw new Exception("Bug in remove method.");
                    }
                } else if (randomInt == 3) {
                    int n4Rand = rand.nextInt(n4 - 1);
                    ...
                    if (set.contains(n4Rand) != refSet.contains(n4Rand)) {
                        throw new Exception("Bug in contains method.");
                    }
                } else {
                    throw new Exception("Not a valid random Integer.");
                }

                Iterator<Integer> iter = refSet.iterator();
